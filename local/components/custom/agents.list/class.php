...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    private function initCache(): void
    {
        ...
        $this->cacheKey = self::class . '_' . md5(json_encode($this->arParams) . json_encode($_GET)); // тут указывается от каких параметров зависит кэш
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
            ...
            $enumRes = CUserFieldEnum::GetList(array(), array('USER_FIELD_ID' => $fieldID));
            while($arList = $enumRes->Fetch())
            {
                $result[$arList['ID']] = $arList['VALUE'];
